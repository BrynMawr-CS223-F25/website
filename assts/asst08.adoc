...
:title: Assignment 9: Personal Shell
:Author Initials: ASN
:Date:    2020-08-15
:Revision: 0.1
:stem: latexmath
...
NOTE: You can print with colors to the console using ANSI escape sequences. 
For example, `printf(\x1b[32m "Green Text" \x1b[0m);` will print green text. link:https://azrael.digipen.edu/~mmead/www/mg/ansicolors/index.html[Here is a good guide!]
...
** (0.1 points) style and header comment
** (1.4 points) Displays unique prompt with current working directory
** Uses `readline()` and `add_history()`
** Quits when the user types `exit`.
** Forks a process to run the user's command. 
** Reports the return status of the command when the child process completes.
** Has a unique prompt appearance
