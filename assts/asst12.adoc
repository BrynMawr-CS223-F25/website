...
Due Friday, April 25th, before midnight. **Written portion is due April 24th - no late work accepted**
...
In `memstats.c`, implement a program that simulates a series of memory 
allocations and then outputs an image that visualizes the memory usage. 
...
== README (*Due in class as a hardcopy*)
