...
Apply glow to an image requires three steps:

* Extracting bright areas into a separate image
* Blurring the bright image
...
then saved a white pixel to a new image if the pixel exceeded the threshold.
...
[stem]
++++
newPixel_{i,j} = (p_{i-1,j-1} + p_{i,j-1} + p_{i+1,j-1} 
...
box size of 49. In other words, we used the 24 neighbors that were left, right, above, and below of the current 
...
The glow effect is achieved by adding the blurred image with the original image.

[stem]
++++
newPixel_{i,j} = originalPixel_{i,j} + blurPixel_{i,j}
++++
