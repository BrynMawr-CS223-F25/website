[cols="1,2,4a,2a", options="header"]
|===
| Week 
| Date 
| Agenda
| Resources

//-----------------------------
| 1
| {WEEK01} anchor:week01[]
| *Introductions, Hello C* 

* Topics: Programming at the command line, Makefiles, From Java to C, strings, arrays, function stack, basic pointers
* Read: link:https://diveintosystems.org/singlepage/#_appendix_1_chapter_1_for_java_programmers[Chapter 1: From JAVA to C] 
* link:assts/asst01.html[Assignment 01]: Due {ASST01_DUE}
* Lab: SSH and Github setup

| 
* link:assts/ssh-setup.html[SSH Setup]
* link:http://linuxcommand.org/lc3_learning_the_shell.php[Learning the shell]
* link:lectures/Lecture-Week01-Sept2-Intro.pdf[Lecture - Sept 2]
* link:lectures/Lecture-Week01-Sept4-C.pdf[Lecture - Sept 4]

//-----------------------------
| 2 
| {WEEK02} anchor:week02[]
| *More C, Pointers* 

* Topics: More pointers, malloc/free, heap, file I/O, gdb, valgrind, hexedit
* Read: link:https://diveintosystems.org/singlepage/#_a_deeper_dive_into_c_programming[Sections 2.1-2.8, 2.9 (Command line arguments only)] 
* Read: link:https://diveintosystems.org/singlepage/#_c_debugging_tools[Sections 3.1-3.3] 
* link:https://pythontutor.com/c.html#mode=edit[C Tutor]
* Lab: Congrats to team "On a R.O.L" as this week's worksheet champions!
* link:assts/asst02.html[Assignment 02]: Due {ASST02_DUE}

|
* link:lectures/Lecture-Week02-Sept9-Pointers.pdf[Lecture - Sept 9]
* link:lectures/Lecture-Week02-Sept11-Pointers.pdf[Lecture - Sept 11]
* link:lectures/code/pass_by_val.c[struct_by_pointer.c]
* link:lectures/code/pass_by_pointer.c[struct_by_value.c]

//-----------------------------
|3
|{WEEK03} anchor:week03[]
|*Data structures in C* 

* Topics: linked lists, trees, editor design and implementation
* Read: link:http://svslibrary.pbworks.com/f/Data+Structures+and+Algorithm+Analysis+in+C+-+Mark+Allen+Weiss.pdf[Data Structures in C, Mark Allen Weiss, Chpt 3]
* Lab: Coding Challenge 01, link:studyguide1.html#_coding[Coding problems 1]

| 
* link:lectures/Lecture-Week03-Sept16-FileIO.pdf[Lecture - Sept 16]
* link:lectures/Lecture-Week03-Sept18-DataStructures.pdf[Lecture - Sept 18]
* link:lectures/code/bigfish.c[bigfish.c]
* link:lectures/code/badprog.c[badprog.c]

//-----------------------------
|4
|{WEEK04} anchor:week04[]
|*Binary and Data Representations* 

* Topics: Designing data structures in C
* Topics: number systems, integer/float/char binary formats, struct layouts, endianess
* Read: link:https://diveintosystems.org/singlepage/#_binary_and_data_representation[Chapter 4.1-4.9] 
* link:assts/asst03.html[Assignment 03]: Due {ASST03_DUE}
* Lab: Check-ins
* Congrats to this week's worksheet champions: Stackers and Turtles 

|
* link:lectures/Lecture-Week03-Sept18-DataStructures.pdf[Lecture - Sept 23 (Same notes as Sept 18)]
* link:lectures/Lecture-Week04-Sept23-Binary.pdf[Lecture - Sept 25]
* link:lectures/code/linkedList/linkedlist.h[linkedlist.h]
* link:lectures/code/linkedList/linkedlist.c[linkedlist.c]
* link:lectures/code/linkedList/test_list.c[test_list.c]
* link:lectures/code/binary_tree/binary_tree.h[binary_tree.h]
* link:lectures/code/binary_tree/binary_tree.c[binary_tree.c]
* link:lectures/code/binary_tree/test_tree_main.c[test_tree_main.c]
* link:lectures/code/string.c[string.c]

//-----------------------------
|5
|{WEEK05} anchor:week05[]
|*Bitwise operators, Introduction to Assembly* 

* Topics: bitwise operators, shifts, masking, working with bit fields
* Read: link:https://diveintosystems.org/singlepage/#_what_von_neumann_knew_computer_architecture[Section 5, 5.1]
* Read: link:https://diveintosystems.org/singlepage/##_assembly_chapter[Chapter 6] 
* Read: link:https://diveintosystems.org/singlepage/#_x64_assembly_chapter[Chapter 7 (7.1-7.3,7.5,7.10), 10] 
* Lab: Coding Challenge 02, link:studyguide2.html#_coding[Coding problems 2]

| 
* link:lectures/Lecture-Week05-Sept30-BitOps.pdf[Lecture - Sept 30]
* link:lectures/Lecture-Week05-Oct2-IntroAssembly.pdf[Lecture - Oct 2]
* link:lectures/code/bitops.c[bitops.c]
* link:lectures/code/byte-order.c[byte-order.c]
* link:lectures/code/shift.c[shift.c]

//-----------------------------
|6
|{WEEK06} anchor:week06[]
| *Assembly, Review*

* Topics: instruction set architecture, machine code, memory forms, x86_64 instructions
* Lab: Systems Quizzo
* Midterm 01: In-class {MIDTERM01_WK6} (80 minutes)
* link:assts/asst04.html[Assignment 04]: Due {ASST04_DUE}

| 
* link:lectures/Lecture-Week06-Oct7-Assembly.pdf[Lecture - Oct 7]
* link:studyguide1.html[Study Guide 1]
* link:studyguide2.html[Study Guide 2]
* link:studyguide3.html[Study Guide 3]

//-----------------------------
|7
|{WEEK07} anchor:week07[]
|*Break*

* *NO LECTURES, LABS, HOMEWORKS*

| 

//-----------------------------
|8
|{WEEK08} anchor:week08[]
|*Assembly* 

* topics: Tracing assembly, buffer exploits, loops, conditionals 
* Read: link:https://diveintosystems.org/singlepage/#_the_operating_system[Chapter 13, skip section 13.3] 
* Lab: Check-ins, worksheet practice. Congratulations to last week's worksheet champions: JE^2^, U.G., and NY.
* link:assts/asst04.html[Assignment 04]: Office hour check-ins due 

| 
* link:lectures/Lecture-Week08-Oct21-AssemblyPractice.pdf[Lecture - Oct 21]
* link:lectures/Lecture-Week08-Oct23-AssemblyBranch.pdf[Lecture - Oct 23]

//-----------------------------
|9
|{WEEK09} anchor:week09[]
| *The operating system, processes, fork*

* Topics: what is an operating system?, system calls, processes, fork
* Lab: Coding Challenge 03, link:studyguide3.html#_coding[Coding problems 3]
* link:assts/asst04.html[Assignment 04]: Due {ASST04_DUE}

| 
* link:lectures/Lecture-Week08-Oct23-AssemblyBranch.pdf[Lecture - Oct 28]
* link:lectures/Lecture-Week09-Oct30-OS-Processes.pdf[Lecture - Oct 30]

//-----------------------------
|10
|{WEEK10} anchor:week10[]
|*Processes, Threads I* 

* Topics: threads vs. processes, designing concurrent algorithms, pthreads
* Read: link:https://diveintosystems.org/singlepage/#_some_advanced_c_features[Section 2.9.3, 2.9.4 (void* andpointer arithmetic)] 
* Read: link:https://diveintosystems.org/singlepage/#_leveraging_shared_memory_in_the_multicore_era[Chapter 14] 
* link:assts/asst05.html[Assignment 05]: Due {ASST05_DUE}
* Lab Activity: Assembly programming

| 
* link:lectures/Lecture-Week10-Nov4-Threads.pdf[Lecture - Nov 6]
* link:lectures/code/threads-find-min.c[threads-find-min.c]

//-----------------------------
|11
|{WEEK11} anchor:week11[]
|*Threads II* 

* topics: performance, deadlock, thread safety, producer/consumer pattern
* Lab: Coding Challenge 04, link:studyguide4.html#_coding[Coding problems 4]
* link:assts/asst06.html[Assignment 06]: Office hours check-ins due 

| 
* link:lectures/Lecture-Week11-Nov11-ThreadsConcurrency.pdf[Lecture - Nov 11]
* link:lectures/Lecture-Week11-Nov13-ThreadPerformance.pdf[Lecture - Nov 13]
* link:lectures/code/threads-badid.c[threads-badid.c]
* link:lectures/code/threads-count.c[threads-count.c]

//-----------------------------
|12
|{WEEK12} anchor:week12[]
|*Memory, memory allocation* 

* Read: link:https://diveintosystems.org/singlepage/#_storage_and_the_memory_hierarchy[Chapter 11.1-11.3] 
* Lab: Checkins and worksheet
* link:assts/asst06.html[Assignment 06] Due Friday Nov 21

| 
* link:lectures/Lecture-Week12-Nov18-MemoryHierarchy.pdf[Lecture - Nov 18]
* link:lectures/Lecture-Week12-Nov20-Malloc.pdf[Lecture - Nov 20]
* link:lectures/code/dining-philosophers.c[dining-philosophers.c]

//-----------------------------
|13
|{WEEK13} anchor:week13[]
|*Memory allocation* 

* Lab: Threads 
* Read:"My malloc: mylloc and mhysa" by Johan Montelius (see slack)
* Read: link:https://diveintosystems.org/singlepage/#_storage_and_the_memory_hierarchy[Chapter 11.1-11.3] 
* **Thanksgiving: No Class Thursday**

| 

//-----------------------------
|14
|{WEEK14} anchor:week14[]
| Review and midterm 2

* Midterm 02, In-class {MIDTERM02} (80 minutes)

| 

//-----------------------------
|15
|{WEEK15} anchor:week15[]
|*Code Optimization, C++*

* Read: link:https://diveintosystems.org/singlepage/#_code_optimization[Chapter 12] 
* link:assts/.html[Assignment 07]: Due {ASST07_DUE}
* Lab checkins and final practice

|
* link:studyguide-final.html[Final Study Guide]

|===


